 ...
 */
function generateTeamPattern(day, teamIndex, randomOffset) {
    const patterns = [
        // Red: Strong early year, fades mid-year, recovers
        (d) => {
            if (d < 100)
                return 1.5 + Math.sin(d / 20) * 0.5 + randomOffset * 0.8;
            if (d < 250)
                return 3.5 + Math.sin(d / 15) * 1 + randomOffset * 0.6;
            return 2 + Math.sin(d / 25) * 0.8 + randomOffset * 0.7;
        },
        // Blue: Consistent mid-tier performer
        (d) => 3 + Math.sin(d / 30) * 0.5 + randomOffset * 0.9,
        // Yellow: Weak start, improves over time
        (d) => 5.5 - (d / 365) * 2 + Math.sin(d / 20) * 0.5 + randomOffset * 0.7,
        // Green: Dominant mid-year
        (d) => {
            if (d < 120 || d > 280)
                return 4 + Math.sin(d / 15) * 0.8 + randomOffset * 0.8;
            return 1.5 + Math.sin(d / 10) * 0.3 + randomOffset * 0.6;
        },
        // Orange: Sporadic performance
        (d) => 4 + Math.sin(d / 8) * 1.5 + randomOffset * 1.0,
        // Purple: Generally lower ranks
        (d) => 5 + Math.sin(d / 40) * 0.7 + randomOffset * 0.8,
    ];
 ...
 */
function scorestoRankings(scores) {
 ...
 */
export function generateDayData(date = new Date()) {
    const data = [];
    ...
    for (let minute = 0; minute < 1440; minute++) {
        ...
        const ranks = isFuture ? [null, null, null, null, null, null] : scorestoRankings(scores);
        data.push({
            timestamp,
            rankings: {
                red: ranks[0],
 ...
 */
export function generateYearData(year = new Date().getFullYear()) {
    const data = [];
    // Generate random offsets for each team to vary their performance each time
    const teamOffsets = [
        (Math.random() - 0.5) * 2,
 ...
 */
export function generateYearMonthlyData() {
    const monthlyData = [];
    const now = new Date();
