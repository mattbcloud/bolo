...
class FirebaseService {
     ...
     */
    async initialize(hostname, serviceAccountPath) {
        if (this.initialized) {
     ...
     */
    async recordMinuteData(timestamp, rankings) {
     ...
     */
    async recordHourlyData(timestamp, averageRanks) {
        const date = new Date(timestamp);
        const year = date.getFullYear();
        const month = String(date.getMonth() + 1).padStart(2, '0');
        const day = String(date.getDate()).padStart(2, '0');
     ...
     */
    async recordDailyData(date, averageRanks) {
     ...
     */
    async recordMonthlyData(month, averageRanks) {
     ...
     */
    async getStatsForPeriod(period) {
        const now = new Date();
        switch (period) {
            case 'hour': {
            ...
            case 'week': {
                // Get last 168 hours (7 days) of hourly data - rolling window
                const oneWeekAgo = now.getTime() - (7 * 24 * 60 * 60 * 1000);
                const oneWeekAgoDate = new Date(oneWeekAgo);
                const results = [];
                // Fetch hourly data from all days in the range (up to 8 days to cover full range)
                const currentDate = new Date(oneWeekAgoDate);
                ...
                // Filter to only last 168 hours
                return results.filter(d => d.timestamp >= oneWeekAgo);
            }
            case 'month': {
            ...
            case 'year': {
                ...
                return filtered.filter((_, i) => i % 24 === 0);
            }
            default:
                return [];
        }
