...
class BoloIrc {
  ...
  constructor(options: IRCOptions) {
    ...
    this.client.addListener('privmsg', (m: any) => {
      if ((m.channel = m.params[0]).charAt(0) !== '#') return;
 ...
 */
function createBoloIrcClient(app: any, options: IRCOptions): BoloIrc {
  const irc = new BoloIrc(options);

  const findHisGame = (ident: string) => {
    for (const [gid, game] of Object.entries<any>(app.games)) {
