 ...
 */
export function isNickTaken(holders: Array<{ name?: string }>, nick: string): boolean {
  const wanted = normalizeNick(nick).toLowerCase();
  if (!wanted) return false;
  return holders.some(
    (holder) => typeof holder?.name === 'string' && normalizeNick(holder.name).toLowerCase() === wanted
  );
}
