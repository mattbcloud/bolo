...
export const NEWSWIRE_CHANNEL_LABELS: Record<NewswireChannel, string> = {
  news: 'NEWSWIRE',
  score: 'SCORELINE',
};
 ...
 */
const SCORE_KINDS: ReadonlySet<NewswireKind> = new Set<NewswireKind>([
  'lead_change',
  'position_change',
]);

/** Which channel an event belongs to. Anything not explicitly a score event is news. */
export function channelOf(kind: NewswireKind): NewswireChannel {
 ...
 */
export function teamActor(team: number): NewswireActor {
 ...
 */
export function pillboxActor(team: number | null | undefined): NewswireActor | null {
  if (team == null || team === 255) return null;
 ...
 */
export function actorOf(obj: { name?: string; team?: number | null }): NewswireActor {
  if (obj.name) return { name: obj.name, team: obj.team ?? null };
  if (obj.team != null && obj.team !== 255) return teamActor(obj.team);
  // Only before Tank.spawn() has assigned a team, which is before the tank is in play at all.
  return { name: 'someone', team: null };
}

function seg(t: string): NewswireSegment {
  return { t, team: null };
}

function name(actor: NewswireActor): NewswireSegment {
...
export function updateStandings(
  ...
  margin: number = NEWSWIRE_POSITION_MARGIN
): NewswireStandings {
  const ranked = new Set<number>();
  for (const team of eligible) {
    if (team >= 0 && team < scores.length) ranked.add(team);
  }
  ...
  const swaps: NewswirePositionSwap[] = [];
  ...
  if (previous !== null && !rosterChanged) {
    ...
    for (let i = 0; i < order.length; i++) {
      const riser = order[i];
      const place = i + 1;
      if (placeBefore.get(riser)! <= place) continue; // did not move up

      let faller: number | null = null;
      for (let j = i + 1; j < order.length; j++) {
        const candidate = order[j];
        if (claimed.has(candidate)) continue;
        if (placeBefore.get(candidate)! >= placeBefore.get(riser)!) continue; // was not ahead
        if (placeNow.get(candidate)! <= placeBefore.get(candidate)!) continue; // did not fall
        faller = candidate;
        break;
      }
      ...
      if (faller !== null) claimed.add(faller);
      swaps.push({
        riser,
        riserPlace: place,
        faller,
...
export function newswireMessage(
  kind: NewswireKind,
  actor: NewswireActor,
  other?: NewswireActor | null,
  detail?: NewswirePositionDetail | null
