...
export const TERRAIN_TYPES: TerrainType[] & { [key: string]: TerrainType } = [
...
function createTerrainMap(): void {
  for (const type of TERRAIN_TYPES) {
    (TERRAIN_TYPES as any)[type.ascii] = type;
  }
}
...
export class MapCell {
  ...
  constructor(map: Map, x: number, y: number, options?: { isDummy?: boolean }) {
    ...
    this.x = x;
    this.y = y;
    this.type = (TERRAIN_TYPES as any)['^'];
  ...
  setType(newType: string | number | TerrainType | null, mine?: boolean, retileRadius?: number): void {
    ...
    if (typeof newType === 'string') {
      this.type = (TERRAIN_TYPES as any)[newType];
  ...
  setTile(tx: number, ty: number): void {
    if (this.mine && !this.pill && !this.base) ty += 10;
...
export class Map {
  static CellClass: typeof MapCell = MapCell;
  static PillboxClass: typeof Pillbox = Pillbox;
  static BaseClass: typeof Base = Base;
  static StartClass: typeof Start = Start;

  CellClass: typeof MapCell = MapCell;
  PillboxClass: typeof Pillbox = Pillbox;
  BaseClass: typeof Base = Base;
  StartClass: typeof Start = Start;
   ...
   */
  each(
    cb: (this: MapCell, cell: MapCell) => void,
    sx?: number,
    sy?: number,
    ex?: number,
    ey?: number
  ...
  clear(sx?: number, sy?: number, ex?: number, ey?: number): void {
    this.each(function (this: MapCell) {
      this.type = (TERRAIN_TYPES as any)['^'];
      this.mine = this.isEdgeCell();
    }, sx, sy, ex, ey);
  }
  ...
  findCenterCell(): MapCell {
    let t = MAP_SIZE_TILES - 1,
      l = MAP_SIZE_TILES - 1;
    let b = 0,
      r = 0;

    this.each(function (this: MapCell, c: MapCell) {
      if (l > c.x) l = c.x;
      if (r < c.x) r = c.x;
      if (t > c.y) t = c.y;
      if (b < c.y) b = c.y;
  ...
  dump(options?: { noPills?: boolean; noBases?: boolean; noStarts?: boolean }): number[] {
    ...
    const consecutiveCells = (row: MapCell[], cb: (type: number, count: number, startx: number) => void) => {
    ...
    // Build the header.
    let data: number[] = [];
    for (const c of 'BMAPBOLO') data.push(c.charCodeAt(0));
    ...
    for (const row of this.cells) {
      ...
      consecutiveCells(row, (type: number, count: number, x: number) => {
  ...
  static load(buffer: ArrayLike<number>): Map {
    ...
    while (true) {
      ...
      if (actualDataLen === 0 && y === 0xff && sx === 0xff && ex === 0xff) break;
